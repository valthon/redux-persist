...
function createPersistoid(config) {
  ...
  var update = function update(state) {
    ...
    Object.keys(lastState).forEach(function (key) {
      if (state.get(key) === undefined && passWhitelistBlacklist(key) && keysToProcess.indexOf(key) === -1 && lastState[key] !== undefined) {
...
function persistReducer(config, baseReducer) {
  ...
  var conditionalUpdate = function conditionalUpdate(state) {
    ...
    state.get('_persist').rehydrated && _persistoid && !_paused && _persistoid.update(state);
    return state;
  };

  return function (state, action) {
    var _persist = state.get('_persist');

    var rest = state.delete('_persist'); // $FlowIgnore need to update State type
    ...
    if (action.type === PERSIST) {
      ...
      if (_purge) return restState.merge({
      ...
      if (action.key === config.key) {
        ...
        var _newState = reconciledRest.merge({
          _persist: _objectSpread2({}, _persist, {
            rehydrated: true
          })
        });
    ...
    return conditionalUpdate(newState.merge({
      _persist: _persist
    }));
  };
}
