...
function createPersistoid(config) {
  ...
  var update = function update(state) {
    ...
    Object.keys(lastState).forEach(function (key) {
      if (state.get(key) === undefined && passWhitelistBlacklist(key) && keysToProcess.indexOf(key) === -1) {
...
function persistReducer(config, baseReducer) {
  ...
  return function (state, action) {
    ...
    var rest = state.delete('_persist');
    var restState = rest;
    ...
    return conditionalUpdate(newState.merge({ _persist: _persist }));
  };
}
